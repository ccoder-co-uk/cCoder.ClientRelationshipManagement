...
[CrmUserAuthorize]
public sealed class ProcessTransitionsController(IProcessTransitionOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] ProcessTransition entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] ProcessTransition entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<ProcessTransition> delta, CancellationToken cancellationToken)
    {
        ProcessTransition entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
