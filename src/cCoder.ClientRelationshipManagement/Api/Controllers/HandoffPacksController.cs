...
[CrmUserAuthorize]
public sealed class HandoffPacksController(IHandoffPackOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] HandoffPack entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] HandoffPack entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<HandoffPack> delta, CancellationToken cancellationToken)
    {
        HandoffPack entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
