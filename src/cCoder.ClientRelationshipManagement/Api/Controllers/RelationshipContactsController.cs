...
[CrmUserAuthorize]
public sealed class RelationshipContactsController(IRelationshipContactOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] RelationshipContact entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] RelationshipContact entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<RelationshipContact> delta, CancellationToken cancellationToken)
    {
        RelationshipContact entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
