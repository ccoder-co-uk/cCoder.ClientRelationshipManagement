...
[CrmUserAuthorize]
public sealed class AgentRunsController(IAgentRunOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] AgentRun entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] AgentRun entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<AgentRun> delta, CancellationToken cancellationToken)
    {
        AgentRun entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
