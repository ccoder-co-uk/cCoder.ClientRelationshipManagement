...
[CrmUserAuthorize]
public sealed class ProcessDefinitionsController(IProcessDefinitionOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] ProcessDefinition entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] ProcessDefinition entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<ProcessDefinition> delta, CancellationToken cancellationToken)
    {
        ProcessDefinition entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
