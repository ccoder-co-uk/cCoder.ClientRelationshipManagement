...
[CrmUserAuthorize]
public sealed class ProcessInstancesController(IProcessInstanceOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] ProcessInstance entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] ProcessInstance entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<ProcessInstance> delta, CancellationToken cancellationToken)
    {
        ProcessInstance entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
