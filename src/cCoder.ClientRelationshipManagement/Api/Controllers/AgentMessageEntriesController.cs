...
[CrmUserAuthorize]
public sealed class AgentMessageEntriesController(IAgentMessageEntryOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] AgentMessageEntry entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] AgentMessageEntry entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<AgentMessageEntry> delta, CancellationToken cancellationToken)
    {
        AgentMessageEntry entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
