...
[CrmUserAuthorize]
public sealed class CompanyContactsController(ICompanyContactOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] CompanyContact entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] CompanyContact entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<CompanyContact> delta, CancellationToken cancellationToken)
    {
        CompanyContact entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
