...
[CrmUserAuthorize]
public sealed class AgentAutomationSettingsController(IAgentAutomationSettingOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] AgentAutomationSetting entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] AgentAutomationSetting entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<AgentAutomationSetting> delta, CancellationToken cancellationToken)
    {
        AgentAutomationSetting entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
