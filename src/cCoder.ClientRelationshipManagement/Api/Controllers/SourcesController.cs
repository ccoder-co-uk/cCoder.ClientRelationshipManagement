...
[CrmUserAuthorize]
public sealed class SourcesController(ISourceOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] Source entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] Source entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<Source> delta, CancellationToken cancellationToken)
    {
        Source entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
