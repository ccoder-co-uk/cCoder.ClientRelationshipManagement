...
[CrmUserAuthorize]
public sealed class LeadContactsController(ILeadContactOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] LeadContact entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] LeadContact entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<LeadContact> delta, CancellationToken cancellationToken)
    {
        LeadContact entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
