...
[CrmUserAuthorize]
public sealed class TenantCompanyRelationshipsController(ITenantCompanyRelationshipOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] TenantCompanyRelationship entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] TenantCompanyRelationship entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<TenantCompanyRelationship> delta, CancellationToken cancellationToken)
    {
        TenantCompanyRelationship entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
