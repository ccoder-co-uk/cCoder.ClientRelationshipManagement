...
[CrmUserAuthorize]
public sealed class ImportsController(IImportOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] Import entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] Import entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<Import> delta, CancellationToken cancellationToken)
    {
        Import entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
