...
[CrmUserAuthorize]
public sealed class OpportunitiesController(IOpportunityOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] Opportunity entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] Opportunity entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<Opportunity> delta, CancellationToken cancellationToken)
    {
        Opportunity entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
