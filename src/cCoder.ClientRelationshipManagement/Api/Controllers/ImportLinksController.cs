...
[CrmUserAuthorize]
public sealed class ImportLinksController(IImportLinkOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] ImportLink entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] ImportLink entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<ImportLink> delta, CancellationToken cancellationToken)
    {
        ImportLink entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
