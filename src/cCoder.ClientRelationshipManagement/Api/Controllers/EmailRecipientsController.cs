...
[CrmUserAuthorize]
public sealed class EmailRecipientsController(IEmailRecipientOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] EmailRecipient entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] EmailRecipient entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<EmailRecipient> delta, CancellationToken cancellationToken)
    {
        EmailRecipient entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
