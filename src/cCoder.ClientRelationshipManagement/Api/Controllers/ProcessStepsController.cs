...
[CrmUserAuthorize]
public sealed class ProcessStepsController(IProcessStepOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] ProcessStep entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] ProcessStep entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<ProcessStep> delta, CancellationToken cancellationToken)
    {
        ProcessStep entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
