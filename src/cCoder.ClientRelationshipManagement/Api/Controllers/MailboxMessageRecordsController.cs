...
[CrmUserAuthorize]
public sealed class MailboxMessageRecordsController(IMailboxMessageRecordOrchestrationService orchestration) : ODataController
{
    ...
    public async Task<IActionResult> Post([FromBody] MailboxMessageRecord entity, CancellationToken cancellationToken) =>
        Created(await orchestration.AddAsync(entity, cancellationToken));

    public async Task<IActionResult> Put([FromRoute] Guid key, [FromBody] MailboxMessageRecord entity, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Patch([FromRoute] Guid key, Delta<MailboxMessageRecord> delta, CancellationToken cancellationToken)
    {
        MailboxMessageRecord entity = await orchestration.RetrieveWriteable().SingleOrDefaultAsync(item => item.Id == key, cancellationToken);
